          ...
          $.each(e, function(type, elem) {
            ...
            switch (type) {
              ...
              default:
                ...
                $("[tourism-title]").html(siteJSON.akkaTourism);
                ...
                $("[copyrightText]").html(siteJSON.copyrightText);
...
$.each(headerNameList, function(i, e) {
  ...
  if (i == currentLang) {
    $.each(e, (i, j) => {
      var isDetailPage = $("#projectDetailsPage").length !== 0 ? true : false;
      var path = isDetailPage ? "/" + j.path : j.path;

      $(".header-menu").append(
        `<li class="header-menu__item"><a href="${path}" class="header-menu__link">${j.name}</a></li>`
      );

      $(".footer-menu").append(
        `<li class="footer-menu__item"><a href="${path}" class="footer-menu__link">${j.name}</a></li>`
      );
    });
  }
});
...
$(".projects-submenu").each(function() {
  ...
  var $firstItem = $(".projects-submenu").find("> *:first-child > a");

  var firstItemTitle = $firstItem.html();
  var firstItemLogo = $firstItem.data("logo");
      ...
      success: function(response) {
        $.each(response, function(i, e) {
          if (e.Url == urlPathName) {
            ...
            $("[project-logo]").attr("data-src", e.Logo);
              ...
              if (e.images.includes(",")) {
                ...
                array.forEach(function(e, i) {
                  $(
                    ...
                    `<div class='slider-slide'><img src='${e}' alt='Project Image ${i +
                      1}' /></div>`
                  );
                });
              } else {
